...
function Header(){
  ...
  const handleScan = async () => {
    ...
    try {
      const response = await axios.post("http://127.0.0.1:5000/api/scan", { target });
      setMessage(response.data.message || "Scan started");
    } catch (error) {
  ...
  return(
    ...
    <section className="min-h-[611px] flex flex-col justify-center px-5 py-8 text-white">
      ...
      <div className="flex max-w-lg mb-8 mx-[70px]">
        ...
        <button className="px-6 py-3 bg-gradient-to-tr from-[#9C5656] to-[#9BA155] text-white rounded-r-md hover:bg-red-500"
        onSubmit={handleScan}>SCAN NOW</button>
        <p>{message}</p>
      </div>
    </section>
